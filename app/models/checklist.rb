class Checklist < ActiveRecord::Base
  ...
  def print_dashboard user_id
    task = self.tasks.my_uncompleted(user_id).sorted.first
    pct_complete = task.nil? ? "0%" : "#{task.percent_complete}%"
    return "<div class=\"noload\"><span class=\"loadtext\">#{pct_complete}</span><div class=\"progload\" style=\"width: #{pct_complete};\"></div></div>"
  end
